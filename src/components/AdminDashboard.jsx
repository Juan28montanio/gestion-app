...
function ExecutiveKpi({ label, value, note, icon: Icon, featured = false, tone = "slate", testId }) {
  const tones = {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p data-testid={testId ? `${testId}-label` : undefined} className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">{label}</p>
          <p data-testid={testId ? `${testId}-amount` : undefined} className={`${featured ? "text-4xl" : "text-2xl"} mt-3 font-black text-slate-950`}>{value}</p>
          <p data-testid={testId ? `${testId}-description` : undefined} className="mt-2 text-sm leading-5 text-slate-500">{note}</p>
        </div>
        {Icon ? (
...
function PaymentMethodSummary({ totals }) {
  ...
  return (
    <article data-testid="payment-methods-section" className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 data-testid="payment-methods-title" className="text-base font-bold text-slate-900">Metodos de pago</h3>
          <p data-testid="payment-methods-subtitle" className="mt-1 text-sm text-slate-500">Lectura rapida del recaudo por canal.</p>
        </div>
        <CreditCard size={20} className="text-slate-400" />
      </div>
      <div data-testid="payment-methods-grid" className="mt-5 space-y-4">
        {rows.map((row) => (
          <div key={row.key} data-testid={`payment-method-${row.key}`}>
            <HorizontalMeter label={row.label} value={row.value} maxValue={maxValue} tone={row.color} />
          </div>
        ))}
      </div>
    </article>
...
export default function AdminDashboard({
  ...
  const handleOpenCash = async () => {
    ...
    try {
      ...
      setCashActionError(error instanceof Error ? error.message : "No fue posible abrir la caja. Revisa la configuracion y vuelve a intentar.");
    } finally {
      setIsBusy(false);
    }
  };
    ...
    <>
      <section className="space-y-6">
        ...
        {moduleMode === "cash" && activeFinanceTab === "cash" ? (
          <>
            <section data-testid="cash-module" className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              {cashActionError ? (
                <div data-testid="cash-open-error" role="alert" className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
                  <span data-testid="cash-error-message">{cashActionError}</span>
                </div>
              ) : null}
              <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                <article data-testid="cash-status-card" className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span data-testid="cash-status-badge" className={`rounded-full px-3 py-1 text-xs font-bold ring-1 ${cashHeroStatus.classes}`}>
                      <span data-testid="cash-status-label">{cashHeroStatus.label}</span>
                    </span>
                    {boxOpenDuration ? (
                  ...
                  <h3 data-testid="cash-reconciliation-title" className="mt-4 text-3xl font-black text-slate-950">Cuadre del turno</h3>
                  <div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-3">
                    <span data-testid="cashier-info">Cajero: <strong data-testid="cashier-name" className="text-slate-900">{cashierLabel}</strong></span>
                    <span data-testid="start-time-info">Inicio: <strong data-testid="start-time-value" className="text-slate-900">{openedAtLabel ? openedAtLabel.toLocaleTimeString("es-CO", { hour: "2-digit", minute: "2-digit" }) : "-"}</strong></span>
                    <span data-testid="base-amount-info">Base: <strong data-testid="base-amount-value" className="text-slate-900">{formatCOP(Number(openSession?.opening_amount || openSession?.openingAmount || 0))}</strong></span>
                  </div>
                </article>
                <div data-testid="cash-actions" className="flex flex-wrap gap-2">
                  <button
                    type="button"
                  ...
                  {openSession ? (
                    <button
                      ...
                      data-testid="close-cash-session-button"
            ...
            <section data-testid="cash-metrics" className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <ExecutiveKpi
                featured
                testId="sales-metric"
                tone="green"
              ...
              />
              <ExecutiveKpi
                testId="digital-payments-metric"
                tone="blue"
                icon={CreditCard}
        ...
        <section data-testid="cash-tabs" className="rounded-[28px] border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex gap-2 overflow-x-auto">
            {visibleTabs.map((tab) => (
              <button
                ...
                data-testid={`${moduleMode === "cash" ? "cash" : "finance"}-tab-${tab.id}`}
        ...
        <div className="grid gap-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <button
              ...
              data-testid="cash-close-cancel-button"
              className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700"
            >
              Volver
            </button>
            <button
              ...
              data-testid="cash-close-confirm-button"
