...
export function createProductForm() {
  return {
    ...
    kitchenStationName: "Cocina",
    preparationTime: "0",
    availableForTables: true,
    availableForQuickSale: true,
...
export function buildProductForm(product) {
  ...
  return {
    ...
    taxRate: String(product.pricing?.taxRate ?? ""),
    targetFoodCost: String(product.pricing?.targetFoodCost ?? product.costing?.targetFoodCost ?? 30),
    productType: product.type || product.product_type || "final_product",
    status: product.status || (product.is_available === false ? "inactive" : "active"),
    ...
    linkedTechnicalSheetId:
      ...
      "",
    consumesInventory: Boolean(product.inventory?.consumesInventory),
    inventoryImpactMode: product.inventory?.inventoryImpactMode || "none",
    allowSaleWhenStockLow: product.inventory?.allowSaleWhenStockLow ?? true,
    requiresKitchen: product.operation?.requiresKitchen ?? Boolean(product.costing?.linkedTechnicalSheetId),
    kitchenStationId: product.operation?.kitchenStationId || "kitchen",
    kitchenStationName: product.operation?.kitchenStationName || "Cocina",
    preparationTime: String(product.operation?.preparationTime ?? 0),
    availableForTables: product.operation?.availableForTables ?? true,
    availableForQuickSale: product.operation?.availableForQuickSale ?? true,
    availableForDelivery: product.operation?.availableForDelivery ?? false,
    visibleInPOS: product.operation?.visibleInPOS ?? product.status !== "hidden",
    visibleInMenu: product.operation?.visibleInMenu ?? true,
    isFavorite: Boolean(product.operation?.isFavorite),
    sortOrder: String(product.operation?.sortOrder ?? 0),
    color: product.operation?.color || "",
    icon: product.operation?.icon || "",
    ticketEligible: Boolean(product.ticket_eligible),
    ticketEligibilityType: product.tickets?.ticketEligibilityType || "",
    ticketValueReference: product.tickets?.ticketValueReference || "unit",
    allowedTicketPlans: Array.isArray(product.tickets?.allowedTicketPlans)
      ? product.tickets.allowedTicketPlans.join(", ")
      : "",
